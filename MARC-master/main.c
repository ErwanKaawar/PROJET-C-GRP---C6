...
int main() {
    ...
    printf("y: %d\n", b.pos.y);
    printf("orie : %s\n", _orie[b.ori]);
    int nbValues = 9; // Nombre de valeurs possibles
    int maxDepth = 5; // Profondeur maximale de l'arbre

    // Créer la racine avec une valeur fictive (-1)
    t_node *root = createNode(-1, nbValues, 0);

    // Construire l'arbre
    //buildTree(root, 0, maxDepth, values, nbValues);
    buildTreerandom(root, 0, maxDepth,nbValues);

    // Afficher l'arbre en parcours préfixe (Racine - Gauche - Droite)
    //printTree(root);
    t_node *minNode = findMinLeaf(root,&counter);
    printf("Nombre d'appels de la fonction FindMinLeaf donc sa complexite est : %d\n", counter);
